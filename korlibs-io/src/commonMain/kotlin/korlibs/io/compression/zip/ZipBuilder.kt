...
private suspend fun AsyncInputOpenable.checksum(checksum: SimpleChecksum) = this.openRead().use { it.checksum(checksum) }
private suspend fun AsyncInputStream.checksum(checksum: SimpleChecksum): Int = checksum.computeWithRead { this.read(it) }
private suspend fun AsyncOutputStream.writeFile(source: VfsFile): Long = source.openUse(VfsOpenMode.READ) { this@writeFile.writeStream(this) }
