...
plugins.withType<YarnPlugin> {
    rootProject.the<YarnRootExtension>().resolution("webpack-cli", "5.1.4")
}

// Patch generated package.json files to replace non-existent webpack-cli@6.0.1 with 5.1.4
val patchWebpackCliVersion by tasks.registering {
    val packagesDir = layout.buildDirectory.dir("js/packages")
    doLast {
        packagesDir.get().asFile.walkTopDown()
            .filter { it.name == "package.json" }
            .forEach { file ->
                val text = file.readText()
                if (text.contains("\"webpack-cli\": \"6.0.1\"")) {
                    file.writeText(text.replace("\"webpack-cli\": \"6.0.1\"", "\"webpack-cli\": \"5.1.4\""))
                    logger.lifecycle("Patched webpack-cli version in ${file.path}")
                }
            }
    }
}
afterEvaluate {
    tasks.findByName("kotlinNpmInstall")?.dependsOn(patchWebpackCliVersion)
    tasks.findByName("jsTestPackageJson")?.let { patchWebpackCliVersion.get().mustRunAfter(it) }
    subprojects.forEach { sub ->
        sub.tasks.findByName("jsTestPackageJson")?.let { patchWebpackCliVersion.get().mustRunAfter(it) }
    }
}
