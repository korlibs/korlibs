...
class Win32Socket private constructor(
	...
	companion object {
		...
		operator fun invoke(secure: Boolean): Win32Socket {
			...
			return Win32Socket(socket, Endpoint(IP(0, 0, 0, 0), 0), secure, debug = debug)
		}
		suspend fun connect(host: String, port: Int, secure: Boolean = false) = Win32Socket(secure).apply { connect(host, port) }
		suspend fun bound(host: String, port: Int, secure: Boolean = false) = Win32Socket(secure).apply { bind(host, port) }
	...
	data class Endpoint(val ip: IP, val port: Int) {
		...
		fun toAsyncAddress(): AsyncSocketAddress = AsyncSocketAddress(ip.str, port)
	}
	...
	fun connect(host: String, port: Int) {
		memScoped {
			...
			debug { "sockaddr_in: ${addr.getBytes(sizeOf<sockaddr_in>().toInt()).toHexString()}" }
	...
	fun tryAccept(): Win32Socket? {
		return memScoped {
			...
			if (wouldBlock()) {
				...
				Win32Socket(fd, addr.ptr.reinterpret<sockaddr_in>().toEndpoint(), secure, debug = secure).apply {
					setSocketBlockingEnabled(false)
				}
			}
		}
...
suspend fun Win32Socket.suspendRecvExact(count: Int): ByteArray {
	return ByteArray(count).apply { suspendRecvExact(this) }
}

suspend fun Win32Socket.suspendRecvUpTo(count: Int): ByteArray {
...
suspend fun Win32Socket.accept(): Win32Socket {
	var time = 0L
	while (true) {
...
private fun GetErrorAsString(error: DWORD = GetLastError().convert()): String {
	return memScoped {
