...
private val CHARSET_PROVIDERS_LOCK = reentrantLock()

fun interface CharsetProvider {
    companion object : CharsetProvider {
        override fun invoke(normalizedName: String, name: String): Charset? = platformCharsetProvider.invoke(normalizedName, name)
    }
...
abstract class Charset(val name: String) {
	...
	companion object {
        ...
        fun registerProvider(provider: CharsetProvider) {
            CHARSET_PROVIDERS_LOCK.withLock {
        ...
        fun forName(name: String): Charset {
            ...
            CHARSET_PROVIDERS_LOCK.withLock {
                for (provider in CHARSET_PROVIDERS) {
...
open class SingleByteCharset(name: String, val conv: String) : BaseSingleByteCharset(name) {
    private val maxCode = conv.maxOrNull()?.plus(1)?.code ?: 0
	val v = ByteArray(maxCode).also { it.fill('?'.code.toByte()); for (n in conv.indices) it[conv[n].code] = n.toByte() }

	override fun encode(out: ByteArrayBuilder, src: CharSequence, start: Int, end: Int) {
		for (n in start until end) {
			val c = src[n].code
			out.append(v.getOrElse(c) { '?'.code.toByte() })
		}
	}
...
fun String.encodeToByteArray(charset: Charset, startIndex: Int = 0, endIndex: Int = this.length, throwOnInvalidSequence: Boolean = false): ByteArray =
    toByteArray(charset, startIndex, endIndex)
...
fun ByteArray.decodeToString(charset: Charset, startIndex: Int = 0, endIndex: Int = this.size, throwOnInvalidSequence: Boolean = false): String =
    this.toString(charset, startIndex, endIndex)

// Korlibs-like variants
