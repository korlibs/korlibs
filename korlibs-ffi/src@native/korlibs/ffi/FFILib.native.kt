...
actual inline fun <T> FFIMemory.usePointer(block: (pointer: FFIPointer) -> T): T = this@usePointer.usePinned { block(it.startAddressOf.rawValue) }

actual val FFIMemory.pointer: FFIPointer get() = TODO()

actual fun CreateFFIPointer(ptr: Long): FFIPointer? = ptr.toCPointer<ByteVar>().rawValue

actual val FFI_POINTER_SIZE: Int get() = sizeOf<COpaquePointerVar>().toInt()

actual val FFIPointer?.address: Long get() = this?.toLong() ?: 0L
...
private inline fun <T : CPointed> FFIPointer.offset(byteOffset: Int): CPointer<T> = (this.toLong().toCPointer<ByteVar>() + byteOffset)!!.reinterpret<T>()
...
actual class FFIArena actual constructor() {
    ...
    actual fun allocBytes(size: Int): FFIPointer = arena.alloc(size, 16).rawPtr
