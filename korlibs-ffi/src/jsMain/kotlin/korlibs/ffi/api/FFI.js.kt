...
private fun toBigInt(str: String): BigInt = js("BigInt(str)")

private fun String.toBigInt(): BigInt = toBigInt(this)

private fun Long.toBigInt(): BigInt = this.toString().toBigInt()

private fun BigInt.toLong(): Long = this.toString().toLong()

private fun createView(bigIntAddress: BigInt, size: Int): dynamic = js("new DataView(Deno.UnsafePointerView.getArrayBuffer(Deno.UnsafePointer.create(bigIntAddress), size, 0))")

private fun createView(address: Long, size: Int) = createView(address.toBigInt(), size) ?: error("Can't create view for address=$address")
