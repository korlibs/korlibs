...
import korlibs.image.core.Win32CoreImageFormatProvider
...
import korlibs.image.vector.renderer.BufferedRenderer
...
import platform.gdiplus.GdipCreateLineBrush
...
import platform.gdiplus.GdipSetLinePresetBlend
import platform.gdiplus.GdipSetLineTransform
...
import platform.gdiplus.GpPointF
...
import platform.gdiplus.WrapMode
...
class GdiRenderer(val bitmap: Bitmap32, val antialiasing: Boolean) : BufferedRenderer() {
    ...
    fun Double.ensureY(bitmap: Bitmap): Float = this.toFloat().ensureY(bitmap)
    ...
    private fun createPath(ppath: CArrayPointer<COpaquePointerVar>, vpath: VectorPath?, bitmap: Bitmap): COpaquePointer? {
        ...
        vpath.visitEdgesSimple(
            ...
            { (x0, y0), (x1, y1), (x2, y2), (x3, y3) ->
                GdipAddPathBezier(
                    path,
                    x0.toFloat(), y0.ensureY(bitmap),
                    x1.toFloat(), y1.ensureY(bitmap),
                    x2.toFloat(), y2.ensureY(bitmap),
                    x3.toFloat(), y3.ensureY(bitmap)
                    ...
                    commands.fastForEach { command ->
                        ...
                        when (style) {
                            ...
                            is TransformedPaint -> {
                                ...
                                GdipSetMatrixElements(matrix, transform.a.toFloat(), transform.b.toFloat(), transform.c.toFloat(), transform.d.toFloat(), transform.tx.toFloat(), transform.ty.toFloat())
                                when (style) {
                                    is BitmapPaint -> {
                                        val bmp = style.bmp32
