				...
				when {
					bmp8 != null -> {
                        ...
                        when (header.bitsPerChannel) {
                            1 -> for (n in 0 until width) bmp[col + n, row] = currentRow.getOrElse(n / 8, 0).extract(7 - (n % 8) * 1, 1)
                            2 -> for (n in 0 until width) bmp[col + n, row] = currentRow.getOrElse(n / 4, 0).extract(6 - (n % 4) * 2, 2)
                            4 -> for (n in 0 until width) bmp[col + n, row] = currentRow.getOrElse(n / 2, 0).extract(4 - (n % 2) * 4, 4)
