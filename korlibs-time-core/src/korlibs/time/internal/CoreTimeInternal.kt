...
internal val MONTH_START_DAYS: Array<IntArray> by lazy { Array(2) { leap -> IntArray(13) { Month_days(it + 1, leap != 0) } } }
...
internal fun Month_adjust(month: Int): Int = ((month - 1) % 12) + 1
internal fun Month_days(month: Int, year: Int): Int = Month_days(month, Year_isLeap(year))
internal fun Month_days(month: Int, leap: Boolean): Int = when (Month_adjust(month)) {
    1, 3, 5, 7, 8, 10, 12 -> 31
    2 -> if (leap) 29 else 28
    else -> 30
}
internal fun Month_fromDayOfYear(dayOfYear: Int, leap: Boolean): Int? {
...
internal fun Month_check(month: Int) {
    if (month !in 1..12) throw DateException("Month $month not in 1..12")
}
internal fun Year_isLeap(year: Int): Boolean = (year % 4 == 0) && (year % 100 != 0 || year % 400 == 0)
internal fun Year_daysSinceOne(year: Int): Int = DAYS_COMMON * (year - 1) + Year_leapCountSinceOne(year)
internal fun Year_days(year: Int): Int = Year_days(Year_isLeap(year))
internal fun Year_days(leap: Boolean): Int = if (leap) DAYS_LEAP else DAYS_COMMON
internal const val DAYS_COMMON = 365
internal const val DAYS_LEAP = 366
...
internal fun Year_leapCountSinceOne(year: Int): Int {
    if (year < 1) {
...
internal fun Year_fromDays(days: Int): Int {
