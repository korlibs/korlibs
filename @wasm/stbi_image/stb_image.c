...
void stbi_write_func_mem(void *context, void *data, int size) {
    ...
    char *inp = (char *)data;
    char *out = ctx->currptr;
    for (int i = 0; i < size; i++) *out++ = *inp++;
    ctx->currptr = out;
}

EMSCRIPTEN_KEEPALIVE int stbi_write_to_memory(int type, char *inp, int w, int h, char *out, int out_size, int quality) {
    WriteCtx ctx = { out };
    switch (type) {
        case 0: stbi_write_png_to_func(stbi_write_func_mem, &ctx, w, h, 4, inp, w * h * 4); break;
